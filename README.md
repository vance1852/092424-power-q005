...
- 燃料船预报按设施所在地 IANA 时区冻结到厂窗口：缺失时刻（春季跳时）前移、重复时刻（秋季回拨）取第一次出现，窗口支持跨午夜与 24:00；部分签收只把新增数量结转为库存批次，超时扫描幂等且不冲销已签收货量，窗口与日历版本在发运时冻结并在重启后沿用；
...
健康检查为 `GET /health`。除健康检查外，请求通过 `X-Actor-Id` 携带操作者编号。可用接口覆盖电价、设施、送出线路、停运事件、燃料批次、燃料船预报与签收（`POST /shipments`、`POST /shipments/{id}/receipts`、`POST /shipments/scan`、`GET /shipments/{id}`）、提名、能力分配、送电、负荷情景和审计链。签收请求的 `arrived_at` 必须带时区偏移（如 `2026-11-01T01:30:00-04:00` 或 `Z`），窗口按设施 `timezone` 的 IANA 规则解析。服务重启后，SQLite 中的业务状态、历史版本和发运时冻结的窗口版本会继续保留。
